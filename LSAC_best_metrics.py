...
def main():
    if len(sys.argv) < 2:
        raise ValueError("Usage: python LSAC_guided_regularizer.py <input_image_path>")
    ...
    if depth is None:
        print(f"Warning: could not read depth map: {depth_path}. Continuing with intensity-only guidance.")
    ...
    np.save(os.path.join("OutputImages", prefix + "_lsac2_guided_loss.npy"), loss_history)

    print(f"Saved guided-regularized LSAC to {out_path}")
