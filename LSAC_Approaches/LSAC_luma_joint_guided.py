...
def make_joint_guide(img_bgr, depth_gray=None, guide_mix=0.1):
    """
...
def estimate_lsac_luminance_iterative(
    ...
    Pure iterative luminance LSAC with:
      ...
      - joint-guided regularizer (intensity + depth)
      - no Gaussian seed!! 
    ...
    guide_mix controls the joint guide as always:
        1.0 = intensity only
        0.0 = depth only
        0.5 = equal mix
    """
...
def main():
    ...
    path = sys.argv[1]
    guided_beta = float(sys.argv[2]) if len(sys.argv) >= 3 else 0.60
    ...
    np.save(
        os.path.join("OutputImages", prefix + f"_lsac_luma_loss{tag}.npy"),
        loss_history,
    )

    print(f"Saved joint-guided luma LSAC to {out_path}")
    print(f"guided_beta: {guided_beta:.2f}, guide_mix: {guide_mix:.2f}")
